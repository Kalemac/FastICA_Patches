	...
	throws FastICAException
{
	...
	System.out.println("eigenValues: " + eigenValues.length + " eigenVectors " + eigenVectors.length + ", " + eigenVectors[0].length);
	...
	System.out.println("After Filter: eigenValues: " + eigenValues.length + " eigenVectors " + eigenVectors.length + ", " + eigenVectors[0].length);
	...
	case SYMMETRIC:
		...
		while (( iter < maxIter ) && ( !ready ))
		{
			...
			{
				System.out.println("IC: " + i + " Iter: " + iter);
		...
		for ( int i = 0; i < numICs; ++i )
		{
			...
			while (( iter < maxIter ) && ( !ready ))
			{
				...
				for ( int j = 0; j < n; ++j )
				{
					...
					double gff = conFunction.function(weightedX);
					double gfd = conFunction.derivative(weightedX);
					System.out.println("GFF: " + gff + " GFD: " + gfd);
					beta += weightedX * gff;
					egfd += gfd;
					exgf = Vector.add(exgf, Vector.scale(gff, actualXVector));
				}
